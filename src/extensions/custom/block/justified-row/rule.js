...
class JustifiedRowRule extends Block {
  constructor(type) {
    super(type);
  ...
  parse(context, parent) {
    ...
    const [m_all, m_marker, m_column, m_indent, m_width, m_gap, m_style] = match;
    const indent = this.textRuler.measure(m_indent) + context.input.column();
    const column = this.textRuler.measure(m_column) + context.input.column();
...
class JustifiedRowItemRule extends Block {
  constructor(type) {
    super(type);
    this.pattern_item = /^(((\s*)(\[[:.' ]{2}\]))({.*?})?)(.*)$/;
    this.textRuler = new TextRuler();
  ...
  start(context, parent) {
    if (parent.type !== "justified-row") return false;
...
export { JustifiedRowRule, JustifiedRowItemRule };